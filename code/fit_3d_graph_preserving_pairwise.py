...
def optimize_on_joints(j2d,
    ...
    for o_id, orient in enumerate(orientations):
        ...
        # Zeeshan's full pairwise graph preserving smoothness term
        # Adjacency graph
        # pdb.set_trace()
        adj_matrix = np.zeros((13,13), dtype='bool')
        edge_i = np.array([0,1,2,2,3,3,4,6,7,8,8,9,9,10])
        edge_j = np.array([1,2,3,8,4,9,5,7,8,9,12,10,12,11])
        adj_matrix[edge_i, edge_j] = 1

        # Build distance matrices and compare for j2d and cam respectively
        dist_j2d = distance_matrix(j2d[cids], j2d[cids])
        dist_cam = distance_matrix(cam[smpl_ids], cam[smpl_ids])
        obj_full_graph = lambda w, sigma: (
            w * GMOf( LA.norm(dist_j2d - dist_cam, 'fro'), sigma))
        obj_graph = lambda w, sigma: (
            w * GMOf( (dist_j2d[adj_matrix]*dist_j2d[adj_matrix] - dist_cam[adj_matrix]*dist_cam[adj_matrix]), sigma))
...
#    _LOGGER.info("BODYORIENT %d,%d",body_orient.shape);
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--out_dir',
        default=os.path.join(filepath,'outDir'),
        type=str,
