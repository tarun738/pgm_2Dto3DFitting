...
# 13 Head top       added
        ...
        newcids = cids[:1]
        newcids += cids[5:8]
        newcids += cids[11:len(cids)]
        newsmpl_ids  = smpl_ids[:1]
        newsmpl_ids += smpl_ids[5:8]
        newsmpl_ids += smpl_ids[11:len(smpl_ids)]
        newweights = np.append(weights[:1],weights[5:8])
        newweights = np.append(newweights,weights[11:len(weights)])
#        newcids = cids;
#        newsmpl_ids =smpl_ids;
#        newweights = weights;
#        import pdb; pdb.set_trace()

        # data term: distance between observed and estimated joints in 2D
        obj_j2d = lambda w, sigma: (w * newweights.reshape((-1, 1)) * GMOf((j2d[newcids] - cam[newsmpl_ids]), sigma))
#        pdb.set_trace()
        normVector = lambda vec1, vec2: ( (vec1/np.linalg.norm(vec1)) - (vec2/np.linalg.norm(vec2)) )
        obj_angles = lambda id2d1,id2d2,id2d3,smd1,smd2,smd3,w,sigma: (
                                    w *  GMOf(( normVector(j2d[id2d2],j2d[id2d1])*normVector(j2d[id2d2],j2d[id2d3])) - (normVector(cam[smd2],cam[smd1])*normVector(cam[smd2],cam[smd3])),sigma) )
#        obj_Depth = lambda w, sigma: (w * GMOf(cam., sigma))

        # mixture of gaussians pose prior
        pprior = lambda w: w * prior(sv.pose)

        j2dMap = np.array([[0,1],[1,2],[2,8],[8,7],[7,6],[8,9],[2,3],[5,4],[4,3],[3,9],[9,10],[10,11]]);
        j3dMap = np.array([[8,5],[5,2],[2,17],[17,19],[19,21],[17,16],[2,1],[7,4],[4,1],[1,16],[16,18],[18,20]]);
        jointDistPrior = lambda w,sigma: w * GMOf(np.linalg.norm(cam[j3dMap[:,1]] - cam[j3dMap[:,0]],axis=1) - np.linalg.norm(j2d[j2dMap[:,1]] - j2d[j2dMap[:,0]],axis=1),sigma)
...
#            customObj including angles
#            ww1=w*0.125;
#            #Knee
#            objs['angles2D'] =obj_angles(0,1,2,8,5,2,ww1,100)
#            objs['angles2D1'] =obj_angles(3,4,5,1,4,7,ww1,100)
#            #elbow
#            objs['angles2D2'] =obj_angles(6,7,8,21,19,17,ww1,100)
#            objs['angles2D3'] =obj_angles(9,10,11,16,18,20,ww1,100)
#            #Hip
#            objs['angles2D4'] =obj_angles(2,3,4,2,1,4,ww1,100)
#            objs['angles2D5'] =obj_angles(1,2,3,5,2,1,ww1,100)
#            #shoulder
#            objs['angles2D6'] =obj_angles(7,8,9,19,17,16,ww1,100)
#            objs['angles2D7'] =obj_angles(8,9,10,17,16,18,ww1,100)
#            objs['distances'] = jointDistPrior(1.,100)
#            import pdb; pdb.set_trace()
...
def main(base_dir,
    ...
    for ind, img_path in enumerate(img_paths):
        ...
        img_path = "/Users/abhi/Desktop/2018WS/PGM/project/pose-tensorflow-master/demo/image.png"
        if not exists(out_path):
            ...
            est = np.array([[208.16989231, 551.00971317,   0.95660961],[217.18445921, 440.8237803 ,   0.95034844],[205.75720501, 298.13772893,   0.71672058],[307.52485061, 290.1131922 ,   0.84360039],[330.58974993, 373.4315443 ,   0.89375627],[300.27535349, 447.33919287,   0.93902057],[120.7404387 , 277.65216494,   0.99124628],[163.64975914, 236.40919742,   0.98042434],[204.0266289 , 157.62143636,   0.9864912 ],[319.61337924, 161.08250737,   0.94212252],[345.69401622, 259.36452574,   0.96232331],[372.93859208, 336.22508955,   0.93514031],[258.30469859, 135.7401123 ,   0.99542588],[256.57129288,  69.74710965,   0.98001844]])
            joints = est[:,:2]
            conf = est[:,2]
#            joints = est[:2, :, ind].T
#            conf = est[2, :, ind]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--out_dir',
        default=os.path.join(filepath,'output'),
        type=str,
