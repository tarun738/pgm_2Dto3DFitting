...
#        pdb.set_trace()
        obj_angles = lambda id2d1,id2d2,id2d3,smd1,smd2,smd3,w,sigma: (
                                    w *  GMOf((( (j2d[id2d2]-j2d[id2d1])*(j2d[id2d2]-j2d[id2d3])) - ((cam[smd2]-cam[smd1])*(cam[smd2]-cam[smd3]))), sigma))

#obj_angles = lambda id2d1,id2d2,id2d3,smd1,smd2,smd3,w,sigma: (
#                                                               w *  GMOf(  ((j2d[id2d2]-j2d[id2d1])*(j2d[id2d2]-j2d[id2d3])/(np.linalg.norm(j2d[id2d2]-j2d[id2d1]) *np.linalg.norm(j2d[id2d2]-j2d[id2d3]))) - ((cam[smd2]-cam[smd1])*(cam[smd2]-cam[smd3])/np.linalg.norm(cam[smd2]-cam[smd1])*np.linalg.norm(cam[smd2]-cam[smd3])), sigma))
#        obj_Depth = lambda w, sigma: (w * GMOf(cam., sigma))
        ...
        jointDistPrior = lambda w: w * np.linalg.norm(np.linalg.norm(cam[j3dMap[:,1]] - cam[j3dMap[:,0]],axis=1) - np.linalg.norm(j2d[j2dMap[:,1]] - j2d[j2dMap[:,0]],axis=1))**2
...
#            objs['j2d'] = obj_j2d(1., 100)
            ...
            objs['distPrior'] = jointDistPrior(0.01);
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'base_dir',
        default='/Users/abhi/Desktop/2018WS/PGM/project/symplify/smplify_public',
        nargs='?',
        help="Directory that contains images/lsp and results/lps , i.e."
        "the directory you untared smplify_code.tar.gz")
    parser.add_argument(
        '--out_dir',
        default='/Users/abhi/Desktop/2018WS/PGM/project/symplify/smplify_public/output',
        type=str,
